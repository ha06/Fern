...
def getCrossOverPopulation(pop = None):
	...
	for k in range(0, math.ceil(global_configuration.pc*len(pop) / 2)):
		...
		#print ("ATTEMPT CO:",popc[k][0].position )
		#popc[k][0].cost  = Fitness((popc[k][0].position, 1))
		#popc[k][1].cost = Fitness((popc[k][1].position, 1))
		#popc[k][1].cost = Fitness(popc[k][1].position,getCost(pop, index = len(pop) - 1))
		#print(popc[k][0].cost, popc[k][1].cost)
		#print(popc[k].cost)
	flatten_popc = []
	for each_elem in popc:
		for sub_elem in each_elem:
			flatten_popc.append(sub_elem)
	print("after flatt:", flatten_popc)
	return flatten_popc
