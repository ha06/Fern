    ...
    for i in range(0, global_configuration.npop):
        ...
        for j in range(global_configuration.varsize[0], global_configuration.varsize[1]):
            if 0 <= j <8:
                ...
                result.append(random_generator(global_configuration.varmin_param_1_2,
                                                       global_configuration.varmax_param_1_2))
            else:
                result.append(random_generator(global_configuration.varmin_param_3_4,
...
def createOutputDir(root_output_dir, it):
    ...
    current_output_dir = os.path.join(root_output_dir, str(it))
...
def mainFunc():
    ...
    with codecs.open(os.path.join(root_output_dir, "fractal_dimension.txt"), mode="w+",encoding="utf-8") as dimension_writer:
        for it in range(1, global_configuration.maxit+1):
            ...
            pop=logPopulationInfo(current_output_dir, it, logger, pop)
            if len(pop) < 10:
                Logger.logger_dev.info("length of population: " + str(len(pop)))
                sys.exit()
            ...
            Logger.logger_dev.info("length of crossover population: " + str(len(popc)))
            Logger.logger_dev.info("dimension of crossover population: " + str(len(popc[0].position)))
            for i in range(0, len(popc)):
                logger.log_crossover_pop(current_output_dir, str(popc[i].position), str(popc[i].cost), str(it))
            Logger.logger_dev.info("Number of Crossed over children:" + str(len(popc)))
            popm = getMutationPopulation(pop=pop)
            Logger.logger_dev.info("length of mutation population: " + str(len(popm)))
            Logger.logger_dev.info("dimension of mutated population: " + str(len(popc[0].position)))
            ...
            pop = pool.map(Fitness, [(datum, global_index + index) for index,y datum in enumerate(pop)])
            # sort the population according to cost
