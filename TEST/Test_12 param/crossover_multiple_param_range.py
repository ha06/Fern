...
def getCrossOverPopulation(pop=None):
    ...
    for k in range(0, math.ceil(global_configuration.nc / 2)):
        ...
        if useRandomSelection is not None:
            i1 = random.randint(0, global_configuration.npop - 1)
            i2 = i1
            while i2 == i1:
                i2 = random.randint(0, global_configuration.npop - 1)
                ...
                popc[k][0].position, popc[k][1].position = np.concatenate((np.concatenate((result0_0_8, result0_8_10)), result0_10_12)), \
                                                           np.concatenate((np.concatenate((result1_0_8, result1_8_10)), result1_10_12))
                print(popc[k][0].position)
                print(popc[k][1].position)
...
def Crossover(x1, x2, gamma, VarMin, VarMax):
    ...
    y1 = [each if np.isnan(each) is False else (random.randrange(VarMin * 100, VarMax * 100) / 100) for each in y1]
    oldMin = np.min(y1)
    oldMax = np.max(y1)
    range_y1 = ((VarMax - VarMin) / (oldMax - oldMin))
    y1 = (((y1 - oldMin) * range_y1))
    y2 = np.dot(alpha, x2) + np.dot((1 - alpha), x1)
    y2 = [each if np.isnan(each) is False else (random.randrange(VarMin * 100, VarMax * 100) / 100) for each in y2]
    oldMin = np.min(y2)
    oldMax = np.max(y2)
